...
import { 
  LayoutDashboard, Search, BookOpen, ShieldCheck, Settings, User, LogOut 
} from 'lucide-react'; 
...
const Sidebar = () => {
  ...
  return (
    <div className="w-64 bg-white h-screen border-r border-slate-100 flex flex-col fixed left-0 top-0 z-50">
      <div className="p-8">
        <h1 className="text-xl font-black text-slate-900 tracking-tighter uppercase">Edunova</h1>
      </div>

      <nav className="flex-1 px-4 space-y-2 mt-4">
        {/* Render Student Links */}
        {role === 'student' && studentLinks.map((item) => (
      ...
      <div className="p-4 mt-auto border-t border-slate-50 space-y-1">
        ...
        {role !== 'admin' && (
          <Link to="/profile" className="flex items-center p-3 text-slate-600 font-semibold hover:bg-slate-50 rounded-xl transition-colors group">
            <User className="mr-3 w-5 h-5 text-slate-400 group-hover:text-blue-600" />
            <span>My Profile</span>
          </Link>
        )}
        
        <button onClick={handleLogout} className="w-full flex items-center p-3 text-red-500 font-semibold hover:bg-red-50 rounded-xl transition-colors group">
...
const SidebarLink = ({ item, activePath }) => {
  ...
  return (
    ...
    >
      <Icon className={`mr-3 w-5 h-5 ${isActive ? 'text-white' : 'text-slate-400'}`} />
      <span>{item.name}</span>
    </Link>
  );
};
